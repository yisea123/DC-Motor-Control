...
//	//中断优先级NVIC设置
	NVIC_InitStructure.NVIC_IRQChannel = TIM7_IRQn;  //TIM27中断
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;  //先占优先级0级
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 3;  //从优先级3级
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE; //IRQ通道被使能
	NVIC_Init(&NVIC_InitStructure);  //初始化NVIC寄存器
...
void TIM7_IRQHandler(void)   //TIM2中断
{
	if (TIM_GetITStatus(TIM7, TIM_IT_Update) != RESET)  //检查TIM2更新中断发生与否
	  ...
	  now_current1=Get_Adc_Average(ADC_Channel_10,3);  //AD采样,3*5=15次约需要0.5ms
  	
		now_current2=Get_Adc_Average(ADC_Channel_11,3);  
	
		now_current3=Get_Adc_Average(ADC_Channel_12,3);	
		}
}
